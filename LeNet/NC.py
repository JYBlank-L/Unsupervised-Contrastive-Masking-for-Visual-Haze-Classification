...
def NC_LeNet(number_of_class):
    ...
    class LeNet5(nn.Module):
        ...
        def forward(self, img):
            ...
            target_region = self.c1(filter)
            target_region_x = self.c2_1(target_region)
            target_region = self.c2_2(target_region)
            target_region += target_region_x
            target_region = self.c3(target_region)
            ...
            similarity = dist(output, target_region)
...
def createModel(number_of_class):
    model = NC_LeNet(number_of_class)
    return model
